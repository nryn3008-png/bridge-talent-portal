...
│   │       ├── personio-client.ts         ← Personio public XML job feed client (+ HTML fallback)
...
- **Portfolio ATS config** — Static mapping in `src/lib/sync/ats-config.ts`. 28 total entries: 19 with active ATS providers (Workable: 8, Personio: 6, Ashby: 3, Lever: 1, Greenhouse: 1) + 9 `manual_only`. Provider union supports 13 ATS types + `fallback` + `manual_only`. Add new entries when portfolio companies adopt ATS with public APIs.
...
- **Fallback scraper** — `src/lib/sync/fallback-scraper.ts` handles careers pages without a known ATS. Exports `scrapeWithFallback(careersUrl, companyName): Promise<RawJob[]>`. 4-step strategy: (1) static HTML fetch + cheerio (JSON-LD, scored DOM elements, link extraction), (2) Playwright network interception for JSON APIs, (3) Playwright DOM extraction on rendered HTML, (4) graceful empty return. Steps 2-3 gated behind `ENABLE_PLAYWRIGHT=true` env var. Uses `playwright-core` (no bundled browser) — connects to `PLAYWRIGHT_WS_ENDPOINT` or local Chromium. Returns `RawJob` (title, location, department, url, rawHtml) — NOT `MappedJobData`. The calling orchestrator handles normalization. Never throws — always returns `[]` on failure. **Three layers of junk protection:** (a) `isSoft404Page()` detects soft 404 pages (HTTP 200 with "not found" content in title/headings/short body), (b) `hasPlausibleJobTitles()` rejects extracted results where <25% of titles match job keywords (catches non-careers pages like company homepages), (c) `sanitizeTitle()` strips HTML tags and base64 data URIs from extracted titles.
- **Fallback scraper careers URLs** — Discovery tries 8 URL patterns per domain: `/careers`, `/jobs`, `/career`, `/join-us` (with and without `www.`), plus `/en/career` and `/en/careers` (language-prefixed). Defined in both `ats-discovery.ts` (`tryFallbackScraper()`) and `job-sync.ts` (`fetchJobsFromProvider('fallback')`).
- **Personio HTML fallback** — `personio-client.ts` supports two modes: (a) XML feed at `/{slug}.jobs.personio.de/xml?language=en` (preferred, requires company to enable it), (b) HTML parsing at `/{slug}.jobs.personio.de/?language=en` extracting from `data-job-position-*` attributes (fallback when XML feed is disabled). Both `fetchPersonioJobs()` and `tryPersonioDiscovery()` try XML first, then HTML. Companies with non-guessable slugs (e.g., `onyo-gmbh-1` for `onyo.io`) must be added to `ats-config.ts` manually.
